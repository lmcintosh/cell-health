...
%    $Revision: 1.1 $  $Date: 2007-03-21 15:56:20 $
...
if flag
   ...
   if isempty(num_of_blocks)
      try
        ...
        error('daq:daqread:cannotread', ['The data file cannot be read.  The following error occurred while reading the file:\n' cacheerror.message]);
      end
   end
...
try
    ...
    % there are other possiblereasons.
    % here's the improved error message, with
    % the read error message appended.
    error('daq:daqread:cannotread', ['The data file cannot be read.  The following error occurred while reading the file:\n' cacheerror.message]);
end;   
...
if nargout >= 0 && all(sampleTimes ~= -1)
   ...
   try
      [data, nanLoc] = localReadData(fid,chart,num_chans,samples,channels,objinfo);
       ...
       % localReadData.
       if strcmp(cacheerror.identifier,'MATLAB:pmaxsize')
           ...
           error('daq:daqread:filetoolarge', 'The file is too large to be read into the program.\nTry specifying a sample range using SAMPLES.');
       else
