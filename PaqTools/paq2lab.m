...
% Usage: [data, names, units, rate,filename]=paq2lab;
%
%
% Adam Packer
% January 30th, 2007
...
% changes by PK 
% Feb 17th, 2009
% a fourth variable input can now be used which specifies the time
% period (in ms) to be extracted from the .paq file. this must be
% specified by a vector of the form [starttime,stoptime]. this
% bypasses the user dialog box when rangequest = 1 asking the length of the
% recording that you want
...
switch secondinput
    case 'channels'
        ...
        if rangequest==1;
            try
                starttime = varargin{4}(1);
                stoptime = varargin{4}(2);
            catch
                prompt1=['Filesize(MB) = ',...
                    sprintf('%-.2f\n',filesize/10^6),...
                    '1sec data (MB) = ',...
            ...
            startbyte=floor(startsample*numchans);
            stopbyte=ceil(stopsample*numchans);
